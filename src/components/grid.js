...
export default class Grid extends Component {
    ...
    createGrid() {
        ...
        for(var i = 1; i <= h; i++) { //h is 20, so i want 20 rows
            for(var j = 1; j <= w; j++) { // w is 10
                var cell = 0;
                row.push(cell);
            }
            grid.push(row);
    ...
    rotate() {
        ...
        var points = [];
        var previous = [];
        for(var i = 4; i < 24; i++) { //h is 20, so i want 20 rows
            for(var j = 0; j < 10; j++) { // w is 10
    ...
    checkRowsToClear() {
        ...
        if(row_was_cleared) {
            this.setState({score: this.state.score + 10 * num_rows_cleared});
        }
    }
    ...
    tick() {
        ...
        for(var i = 23; i >= 0; i--) { //h is 20, so i want 20 rows
            for(var j = 9; j >= 0; j--) { // w is 10
                if(belongs(this.checkColor(i,j))){
                    points.push({i, j});
                }
            }
        ...
        return (
            <View style={{flex: 1, justifyContent: 'space-around'}}>
                <View style={{paddingTop: 40, justifyContent: 'center', alignItems: 'center'}}>
                    <Text style={{fontWeight: '700', fontSize: 26}}> Game xếp hình</Text>
